...
For this repo specifically: a rule change here silently changes how every project is built, so the PR is
where that blast radius gets looked at. Without a PR there is no moment when that question gets asked, and the structure decays regardless of
...
This repo *is* the conventions — see [`README.md`](README.md). Files are symlinked into place by
`install.sh`, so editing the installed file edits this repo. Changing a convention means changing it
here and running `new-project.sh sync <slug> --apply` on every project.
